...
@Component({
  ...
  styles: [`
    ...
    .nf-badge{display:inline-flex;align-items:center;gap:7px;font-size:.72rem;font-weight:800;text-transform:uppercase;letter-spacing:.14em;color:#0F4C81;background:#eff6ff;border:1px solid #dbeafe;padding:6px 15px;border-radius:30px;margin-bottom:24px}
    .nf-code{font-family:'Playfair Display',serif;font-size:clamp(4.5rem,14vw,8rem);font-weight:800;line-height:1;letter-spacing:-.04em;margin:0 0 8px;background:linear-gradient(115deg,#0F4C81,#2563EB,#14B8A6);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
    ...
    .nf-btn-primary{background:linear-gradient(135deg,#2563EB,#0F4C81);color:#fff;box-shadow:0 8px 24px -8px rgba(15,76,129,.6);&:hover{transform:translateY(-2px);box-shadow:0 14px 34px -8px rgba(15,76,129,.7)}}
    .nf-btn-ghost{background:#fff;border:1.5px solid #e2e8f0;color:#334155;&:hover{border-color:#3b82f6;color:#0F4C81;transform:translateY(-1px)}}
    .nf-links{display:flex;gap:8px 24px;justify-content:center;flex-wrap:wrap;padding-top:28px;border-top:1px solid #e2e8f0;a{color:#64748b;font-size:.9rem;font-weight:600;text-decoration:none;transition:color .15s;&:hover{color:#0F4C81}}}
  `]
})
